  ...
  <div>
    <div>
      ...
      <el-form :inline="true" :model="queryCondition" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button type="primary" @click="deleteAllAddress()"
            >批量删除</el-button
          >
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          <el-input
            v-model="queryCondition.contact"
            placeholder="联系人"
          ></el-input>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" @click="newAddAdd" icon="el-icon-search"
            >新增</el-button
          >
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column prop="modifyBy" label="修改者" width="120">
          <template slot-scope="scope">
            <!-- {{scope.row.modifyBy==1?'admin':'zhangsan'}} -->
            {{
              scope.row.modifyBy == 1
                ? "admin"
                : scope.row.modifyBy == 2
                ? "zhangsan"
                : " "
            }}
          </template>
        ...
        <el-table-column prop="roleName" label="创建者角色" width="120">
        </el-table-column>
    ...
    <el-dialog title="添加地址" :visible.sync="saveMark">
      ...
      >
        ...
        >
          <el-input
            v-model="addressObj.addressDesc"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item
        ...
        <el-input
          v-model="addressObj.createdByName"
          autocomplete="off"
          type="hidden"
        ></el-input>
        <el-input
          v-model="addressObj.createdBy"
          autocomplete="off"
          type="hidden"
        ></el-input>
        <!-- </el-form-item> -->
        ...
        <el-date-picke
          v-model="addressObj.creationDate"
          autocomplete="off"
          type="datetime"
        ...
        <el-input
          v-model="addressObj.modifyDate"
          autocomplete="off"
          type="hidden"
        ></el-input>
        <el-input
          v-model="addressObj.roleId"
          autocomplete="off"
          type="hidden"
        ></el-input>
        <!-- </el-form-item> -->
    ...
    <!-- 保存的面板，对话框，模态框 修改地址 -->
    <el-dialog title="修改地址" :visible.sync="updateMark">
      <el-form
        :model="addressObj"
        status-icon
        class="demo-ruleForm"
  ...
  methods: {
    ...
    exportExcel() {
      window.location.href =
        "http://localhost:8080/ssm_market_project/address/exportExcel";
      console.log("aaa");
    },
    ...
    // 查询按钮的功能
    query() {
      this.$postRequest("/address/queryLimit", this.queryCondition).then(
        (res) => {
          if (res) {
    ...
    handleEdit(row) {
      ...
      this.addressObj.id = row.id;
      this.addressObj.contact = row.contact;
      this.addressObj.addressDesc = row.addressDesc;
      this.addressObj.postCode = row.postCode;
      this.addressObj.tel = row.tel;
      this.addressObj.createdBy = row.createdBy;
      this.addressObj.createdByName = row.createdByName;
      this.addressObj.creationDate = row.creationDate;
      this.addressObj.roleId = row.roleId;
      this.addressObj.modifyBy = row.id;
    },
    // 确认修改
    updateForm(formName) {
      // 获取当前时间
      ...
      this.addressObj.modifyDate = nowTimeupdate;
      // 获取修改者的ID
      this.addressObj.modifyBy = this.userObj.id;
      console.log(this.addressObj);
      console.log("aaa");
      console.log(this.addressObj);
      this.$postRequest("/address/updateAddress", this.addressObj).then(
        (res) => {
          this.query();
          this.$message({
            message: "修改成功",
    ...
    handleDelete(row) {
      console.log(row.id);
      this.$deleteRequest("/address/deleteAddressById?id=" + row.id).then(
        (res) => {
          console.log(row.id);
          // 确认取消删除弹框
    ...
    deleteAllAddress() {
      ...
      this.$confirm("确认删除多条供货商吗?", "提示", {
        ...
        .then(() => {
          ...
          console.log(idList);
          this.$deleteRequest("/address/deleteAddresses?ids=" + idList).then(
            (res) => {
              this.query();
            }
          );
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // alert('submit!');
          this.$postRequest("/address/insertAddress", this.addressObj).then(
            (res) => {
              this.saveMark = false;
              this.$message({
                message: "添加成功",
