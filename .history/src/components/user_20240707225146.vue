    ...
    <div>
        ...
        <div style="float: left;">
            <el-form :inline="true" :model="queryCondition" class="demo-form-inline">
                <!-- 导入导出 -->
                <el-form-item>
                    <el-input type="file" @change="handleFileUpload" style="width: 130px;"></el-input>
        ...
        <el-dialog title="修改用户信息" :visible.sync="updateUser">
            <el-form :model="addform">
                <el-input v-model="addform.id" autocomplete="off" type="hidden"></el-input>
                <el-form-item label="用户名" :label-width="formLabelWidth">
                    <el-input v-model="addform.username" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="手机号" :label-width="formLabelWidth">
...
<script>
    export default {
        ...
        methods: {
            ...
            // 导入
            importUsers() {
                this.$postRequest('/user/importExcel').then(res => {
                 
                })
            },
            // 导出
            exportExcel() {
                this.$getRequest('/user/exportExcel').then(res => {
            ...
            handleEdit(row) {
                ...
                if (row.userrole == 1) {
                    this.addform.userrole = '1'
                } else {
                    this.addform.userrole = '2'
                }
            },
            // 确认修改
            submitUser() {
                console.log(this.addform);
                this.$postRequest('/user/updateUser', this.addform).then(res => {
                    this.$message({
                        type: 'success',
                        message: '修改成功!'
                    });
