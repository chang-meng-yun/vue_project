		...
		<el-dialog title="新增订单" :visible.sync="saveMark">
			<el-form :model="BillObj" status-icon class="demo-ruleForm" :rules="rules" ref="ruleForm">
				...
				<el-form-item label="是否支付" :label-width="formLabelWidth" prop="isPayment">
					...
					<template>
						<el-radio v-model="BillObj.isPayment" label="1">已支付</el-radio>
						<el-radio v-model="BillObj.isPayment" label="2">未支付</el-radio>
					</template>
				</el-form-item>
				<!-- 创建者 默认为登录用户 -->
				...
				<el-form-item label="订单日期" :label-width="formLabelWidth" prop="createdDate">
					<el-date-picker v-model="BillObj.createdDate" type="datetime" placeholder="选择日期时间" format = "yyyy-MM-dd HH:mm:ss"></el-date-picker>
				</el-form-item>
			</el-form>
	...
	export default {
		...
		methods: {
			...
			submitForm(formName){
				...
				console.log(this.BillObj.createdDate)
				console.log(typeof(this.BillObj.createdDate))
				// console.log()
				var year = this.BillObj.createdDate.getUTCFullYear();
				var month = this.BillObj.createdDate.getUTCMonth();
				var day = this.BillObj.createdDate.getUTCDate();
				var hours = this.BillObj.createdDate.getHours();
				var minutes = this.BillObj.createdDate.getMinutes();
				var seconds = this.BillObj.createdDate.getSeconds();
				

				var hh = year+"-"+month+"-"+day+" "+hours+":"+minutes+":"+seconds;
				console.log(hh)
				this.BillObj.createdDate=hh;

				// var aa =  new Date(this.BillObj.createdDate).to+' '+new Date(this.BillObj.createdDate).toLocaleTimeString()
				//String转Date
				// console.log(aa)
