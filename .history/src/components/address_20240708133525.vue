<template>
    <div>
                ...
                <el-table-column prop="modifyBy" label="修改者" width="120">
                    <template slot-scope="scope">
						<!-- {{scope.row.modifyBy==1?'admin':'zhangsan'}} -->
                         {{scope.row.modifyBy==1?'admin':(scope.row.modifyBy==2?'zhangsan':' ')}}
					</template>
                </el-table-column>
        ...
        <el-dialog title="修改地址" :visible.sync="updateMark">
            <el-form :model="addressObj" status-icon class="demo-ruleForm" :rules="rules" ref="ruleForm">
                <el-input v-model="addressObj.id" autocomplete="off" type="hidden"></el-input>
                <el-form-item label="联系人" :label-width="formLabelWidth" prop="contact">
                    <el-input v-model="addressObj.contact" autocomplete="off" ></el-input>
                </el-form-item>
                <el-form-item label="地址描述" :label-width="formLabelWidth" prop="addressDesc">
                ...
                <!-- </el-form-item> -->
                <!-- <el-form-item label="修改者" :label-width="formLabelWidth"> -->
                <el-input v-model="addressObj.modifyBy" autocomplete="off" type="hidden">
                </el-input>
                <!-- </el-form-item> -->
                <!-- <el-form-item label="修改时间" :label-width="formLabelWidth"> -->
                <el-input v-model="addressObj.modifyDate" autocomplete="off" type="hidden"></el-input>
...
<script>
    export default {
        ...
        methods: {
                // 导出
                exportExcel(){
               window.location.href="http://localhost:8080/ssm_market_project/address/exportExcel";
               console.log("aaa")
            },
            ...
            updateForm(formName){ 
                ...
                var nowTimeupdate = year+"-"+month+"-"+day+" "+hours+":"+minutes+":"+seconds;
                ...
                console.log("aaa")
                console.log(this.addressObj)
				this.$postRequest('/address/updateAddress', this.addressObj).then(res => {
					this.query();
					this.$message({
						message: '修改成功',
						type: 'success'
