		...
		<el-dialog title="新增订单" :visible.sync="saveMark">
			<el-form :model="BillObj" status-icon class="demo-ruleForm" :rules="rules" ref="ruleForm">
				...
				<el-form-item label="订单日期" :label-width="formLabelWidth" prop="createdDate">
					<el-date-picker  v-model="BillObj.createdDate" type="datetime-local" placeholder="选择日期时间"> </el-date-picker>
				</el-form-item>
	...
	export default {
		...
		methods: {
			...
			submitForm(formName){
				...
				this.BillObj.createdDate = Date (this.BillObj.createdDate).toLocaleDateString()
				console.log(new Date (this.BillObj.createdDate).toLocaleDateString())
